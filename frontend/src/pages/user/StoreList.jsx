...
export function StoreList() {
  ...
  async function reload() {
    try {
      const data = await api(`/stores?${query}`);
  ...
  async function rate(storeId, value) {
    try {
      await api(`/stores/${storeId}/rating`, { method: "PUT", body: JSON.stringify({ value }) });
      reload();
    } catch (e) {
      setErr(e.message);
    }
    ...
    <div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 mb-4 flex gap-3 items-end flex-wrap">
        <div className="flex-1 min-w-[240px]">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" placeholder="Type to filter…" />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div key={s.id} className="bg-white rounded-2xl border border-slate-200 p-5">
            <div className="flex justify-between items-start mb-2">
              ...
              <div className="text-right">
                ...
                <div className="text-amber-600 font-semibold">{s.rating !== null ? `${s.rating.toFixed(2)} ★` : "—"}</div>
            ...
            <div className="mt-4 pt-4 border-t border-slate-100">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-medium text-slate-900">{s.myRating ? `${s.myRating} ★` : "Not rated"}</span>
              </div>
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((v) => (
                  <button key={v} onClick={() => rate(s.id, v)}
                    className={"flex-1 py-1.5 rounded-md text-sm font-medium border transition " +
                      (s.myRating === v ? "bg-amber-500 border-amber-500 text-white" : "bg-white border-slate-300 text-slate-700 hover:bg-amber-50 hover:border-amber-400")}>
                    {v}
                  </button>
                ))}
              </div>
        ...
        {stores.length === 0 && <div className="col-span-full text-center text-slate-400 py-12">No stores found.</div>}
      </div>
    </div>
  );
}

function SortBtn({ children, active, order, onClick }) {
  return (
    <button onClick={onClick} className={"px-3 py-2 rounded-lg border " + (active ? "bg-indigo-50 border-indigo-300 text-indigo-700" : "bg-white border-slate-300 text-slate-700")}>
      Sort: {children}{active && <span className="ml-1 text-xs">{order === "asc" ? "▲" : "▼"}</span>}
    </button>
  );
}
