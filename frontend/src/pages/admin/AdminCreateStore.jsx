...
export function AdminCreateStore() {
  ...
  async function onSubmit(e) {
    e.preventDefault();
    setErr(null);
    const issues = [validateName(name), validateEmail(email), validateAddress(address), ownerEmail ? validateEmail(ownerEmail) : null].filter(Boolean);
  ...
  return (
    <div className="max-w-lg">
      ...
      <form onSubmit={onSubmit} className="bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
        ...
        <Field label="Owner email (optional — promotes existing user to OWNER)" type="email" value={ownerEmail} onChange={setOwnerEmail} optional />
