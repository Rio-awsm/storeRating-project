...
export function AdminUserDetail() {
  ...
  useEffect(() => {
    api(`/admin/users/${id}`).then((d) => setUser(d.user)).catch((e) => setErr(e.message));
  }, [id]);
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-4">
        ...
        {user.role === "OWNER" && (
          <>
            <Row label="Average rating across owned stores" value={user.ownerRating !== null ? user.ownerRating.toFixed(2) : "No ratings yet"} />
            <Row label="Owned stores" value={user.stores.length > 0 ? user.stores.map((s) => s.name).join(", ") : "None"} />
          </>
        )}
      </div>
    </div>
