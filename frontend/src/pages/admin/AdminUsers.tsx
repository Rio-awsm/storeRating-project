...
export function AdminUsers() {
  ...
  useEffect(() => {
    api<{ users: UserRow[] }>(`/admin/users?${query}`)
          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.id} className="border-t border-slate-100">
                ...
                <td className="px-4 py-2">
                  <Link to={`/admin/users/${u.id}`} className="text-indigo-600 hover:text-indigo-700 text-sm">
                    View
                  </Link>
                </td>
              </tr>
            ...
            {users.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-6 text-center text-slate-400">No users match these filters.</td>
              </tr>
            )}
          </tbody>
        </table>
...
function Th({ children, onClick, active, order }: { children: React.ReactNode; onClick: () => void; active: boolean; order: "asc" | "desc" }) {
  return (
    <th
      onClick={onClick}
      className="text-left px-4 py-2 font-medium cursor-pointer select-none hover:text-slate-900"
    >
      {children}
